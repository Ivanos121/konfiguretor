...
Comboboxcheck::Comboboxcheck( const QString& visualColumn, const QString& queryTail, QObject *parent ) :
...
QVariant Comboboxcheck::dataFromParent( QModelIndex index, int column ) const
{
    return QSqlQueryModel::data( QSqlQueryModel::index( index.row() - 1 // "- 1" because make first row empty
                                                        , column ) );
}

int Comboboxcheck::rowCount(const QModelIndex &parent) const
{
    return QSqlQueryModel::rowCount( parent ) + 1; // Add info about first empty row
}

QVariant Comboboxcheck::data(const QModelIndex & item, int role /* = Qt::DisplayRole */) const
{
    QVariant result;
